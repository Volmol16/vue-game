    ...
    methods: {
        ...
        endGame() {
            ...
            if (this.score.zerg > this.score.protoss && this.score.zerg > this.score.terran &&
                this.score.primel < 8 && Math.abs(this.score.protoss - this.score.zerg) > 3) {
                this.goToResult('zerg')
                this.totalGame.zerg++
            } else if (this.score.primel > this.score.protoss &&
                this.score.primel > this.score.terran && this.score.primel == 0) {
                ...
                this.score.protoss > this.score.terran && this.score.taldarim == 0) {
            ...
            localStorage.setItem('sc2TotalGme', JSON.stringify(this.totalGame))
        }
    }
})

let audio = new Audio('audio/soundtrack.mp3');
let audioBnt = document.querySelector('.btn__sound');
audioIcon = document.querySelector('.btn__sound i');
...
audioBnt.addEventListener('click', function () {
    if (audio.muted) {
        audio.muted = false;
